...
- 別系統の仕様（旧v5.0 NAVY配色など）が混在していても、**本スキルが指す `slides_spec.md`（現行 v6.5）を正**とする。
...
- 本SKILL.mdは概要・起動条件・運用ルールを示すメタ文書であり、実装仕様の正は `slides_spec.md`（現行 v6.5、スライドタイプ15種＋考察系＋発見の道筋＋統計予測）にある
...
- **新スライド型**: `add_insight_slide`（4層考察頁）/ `add_patent_micro_slide`（代表特許）/
  ...
  `add_lifecycle_slide`（**技術ライフサイクル**＝全クラスタ＋トピックを累積ロジスティック回帰で一括局面判定）。
