...
- **発明アイディア3ゾーン（`add_invention_zone_slide`）**: 分析結果を出願案に変換する締めの主役級スライド。
  **Hot**＝近年ホット領域に攻めの出願／**Remote**＝ホワイトスペース（遠縁シナジー）／**Battle**＝競合の得意領域に差分軸で攻め込む。
  各ゾーンで「想定独立請求項案（大きく主役）」「発明のポイント（解決課題＋要点）」「先行技術（番号・出願人・クレーム骨格）」「請求項作成ロジック5段」を固定。先行技術は patents.csv から実在番号で選定。暗赤背景画像 `assets/dark_red_background.png` を埋め込む（色は赤/黒/白/グレーのみ）。
