...
## 3.9 v6.24 グラデーション活用＋サブメッセージの仕上げ
- **グラデのブランド統一**: ヘルパ `grad_fill(shape, c1, c2, angle)` を新設。配色は **`ACCENT`(クリムゾン C51212)→`BRIGHT_RED`(F43333)** と **`CRIMSON_DEEP`(8E0014)→`ACCENT`** を標準とする。
  アクセントバー・KPI強調・注目バー等の**“赤を効かせる箇所”に限定**して使う（赤9:1を維持。広い面のグラデは不可）。
- **サブメッセージ `add_sub_message`**: ボックスを**角丸**化、テキストを**オブジェクト内で上下中央寄せ**、左アクセントバーを
  **クリムゾン→明赤の縦グラデ（角丸・上下インセット）**に変更。content 側の呼び出し変更は不要。
- **KPI強調カード**: `emphasis:True` のカードのみ左端に同グラデのアクセントバーを付与（“言いたい数字”が際立つ）。
- **章扉の大番号（D）**: `add_section_slide` の巨大番号を **クリムゾン→深紅の縦グラデ文字**（`_grad_text_fill`）に。テキストの
  グラデは生XML（`a:gradFill`）で適用。content 側の変更は不要。
...
## 3.11 v6.24 章扉の差し色（赤→紫）＋建築面の基本グラデ化
- **章扉オブジェは基本グラデ**: `_cv_base_stage` と `add_section_slide` の建築面（パラレログラム/トライアングル）を
  原則 `_cv_grad_shape`（深→明ランプ）で描画し、立体感を出す（章扉のみなので過剰にならない）。
- **章ごとの差し色（飽き防止）**: `_chapter_accent(section_num, TOTAL_CHAPTERS)` が **赤→紫→赤→紫…** を返す。
  クリムゾンはメインカラーのまま、**偶数章だけ紫ランプ**（7E22A8/4C1577/26093F）に差し替え。
  **最初(1)と最後(TOTAL_CHAPTERS)は強制で赤**。番号グラデ・罫・スキャンライン・建築面すべてに差し色が反映される。

## 3.12 v6.24 紫章は本文もテーマカラー紫
- `add_section_slide` が章扉を描くとき **`_apply_chapter_theme()` で本文用グローバル `ACCENT`/`BRIGHT_RED`/`CRIMSON_DEEP` も切替**。
  以降に追加される本文スライド（表ヘッダのゼブラ・KPI強調カード地/バー・考察ラベル・章ラベル/左バー・章ガイド等）が
  **その章のテーマ色**で描かれる（赤章=クリムゾン、紫章=明るい紫 **#A12FC4**）。次の章扉で再び切り替わる。
- 仕組み: 全関数が参照する**モジュールグローバル ACCENT を章境界で再代入**するだけ（実行順＝文書順のため自動で正しく切替）。
  ハードコードのクリムゾン直値は `_tint(ACCENT, f)` 等で ACCENT 由来に変更済み（KPI強調カード地・アイコン色）。
