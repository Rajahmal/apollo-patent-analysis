...
3. **効く差分軸（専門材料・数値範囲）を特定** — 進歩性の核。先行に無い専門材料／数値パラメータ／構造を**必須構成に1つ以上**。
   - **★最重要（v6.22）＝進歩性は『機構』に紐づける**：差分軸は思いつきの配合でなく、**その分野の劣化・破壊・反応の機構に
     直結した測定可能な限定**にする。「角度は良いが新規性が無い（公知手段の単純組合せ）」を避けるための核。
     **やり方**：①対象材料の“効かない原因＝律速機構”を1つ特定 → ②それを断つ材料/構造を必須構成に → ③その効果を**機構の指標**で数値規定。
     **実証パターン（再生材コンパウンドの例）**:
     - rPET＝**加水分解連鎖**が律速 → 解重合材で末端基をリセット → **カルボキシル末端基(CEG)・ジエチレングリコール(DEG)** で規定。
     - 混合PCR＝**汚染樹脂(PET/PA)が硬質介在物**で破壊起点＋酸化臭 → 反応性二重系(GMA＋カルボジイミド)で**強化相へ転換** →
       **分散径・HS-GCアルデヒド(ヘキサナール)・破断ひずみ**で規定。
     - GF×rPP＝**剛性↑は衝撃↓・密度↑とトレードオフ** → 二重カップリング(MAH-PP＋GMA変性エラストマー) →
       **曲げ弾性率・シャルピー・密度を“同時に満たす”** トライアングルで規定。
   - 単なる「Aを5〜50質量部含む組成物」止まりは弱い。**機構指標（末端基・酸化生成物・物性トライアングル 等）を必須限定に入れる**。
