...
- [ ] ④**由来限定**（「〜由来CO2」等）は**測定可能なフィンガープリント**へ変換（例 放射性炭素≧50pMC／固有共成分 P 0.3〜3%）
- [ ] ⑤**独立項は「機構に紐づくクリティカル限定」だけに絞る（v6.24）**。「〜に由来する」等の測定不能な無駄ワード、一般条件（養生のRH・CO₂分圧等）、具体材料名、付加価値（H₂回収等）は**従属項へ降ろし**、独立項を広く・回避困難にする。数値の羅列（選択発明の弱い進歩性）を避け、各限定を**効果・区別軸**に対応させる。
- [ ] ⑥**物差しは1つに統一**。肥料等は分野の法定基準（K₂O換算等）が最も明確だが、化合物%（全量基準）と酸化物換算を**混在させない**。
...
## 3.13 v6.24 章扉の数字拡大＋右オブジェのメリハリ＋発明レイアウト
- **章扉のメイン数字を約2倍**（118→236pt、ゴーストも220pt）。
  - **ゴーストは背面シャドウ**: 本体より先に描いて背面に置き、右下へオフセットして本体に**約2/3重ねる**（影/エコー表現）。
  - **横線は数字の下**（数字と重ならない位置）へ。数字サイズは変えずに罫/見出し/サブ/SECTIONを下方reflow。
- **右の建築オブジェ**: 4面を横に**ずらし**、回転角（-15/-3/-11/+2）と**透明度（94/64/70/40）**を散らして奥行きを出し、
  さらに**全体を1.5倍に拡大**（上下に少しブリードして迫力を出す）。基本グラデは維持。
- **発明スライド `add_invention_zone_slide`**: 主役の**想定独立請求項案（白＋赤枠）を縦に拡大し最前面**（最後に描画）。
  発明のポイント/先行技術は下方へ移動・小型化、**請求項作成ロジックは極小フォント・高さ半分**の最下部ストリップに。

## 3.14 v6.24 三位一体図の刷新・章導入の可読性・中央寄せ・MD痕跡除去
- **三位一体 `add_triangle_slide` を収束デザインに刷新**: 細い灰色直線の三角（チープ）を廃止。3枚の**角丸カード**
  （色付きヘッダー）を三角配置し、**中央のクリムゾン円ノードへ赤い矢印を収束**させて「相互強化→押上げ」を表現。
  `center=` で中心ノードの語を渡す（例 `center="鉱物化"`）。
- **章導入 `add_chapter_intro_slide` の右箇条書き**: 「**見出し**：説明」は**見出しを太字**にし、**コロン後に改行**して
  説明を次行（ぶら下げインデント）に置く＝可読性向上。左の縦バーは見出し高さに連動（長くなりすぎない）。
- **右寄りの解消**: `CONTENT_W = 13.33 - MARGIN_L*2`（左右マージンを 0.9 に揃え中央寄せ）。`CONTENT_W + 0.5` の右拡張は廃止。
- **MD痕跡 `**` の除去**: `set_text` で `**` を常に除去（保険）。`add_applicant_profile_slide` の箇条は `add_rich_runs` で太字処理。
