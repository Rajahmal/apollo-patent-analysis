...
- **章扉のメイン数字を約2倍**（118→236pt、ゴーストも220pt）。左カラムの罫/見出し/サブ/SECTIONは下方へreflow。
- **右の建築オブジェにメリハリ**: 4面を横に**ずらし**、回転角（-15/-3/-11/+2）と**透明度（94/64/70/40）**を散らして奥行きを出す
  （重なりすぎ・のっぺりを解消）。基本グラデは維持。
