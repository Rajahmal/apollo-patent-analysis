# PPTスライド仕様書 v6.24 — 表紙白基調・グラデ活用・サブメッセージ刷新・章ガイド/コーナー整理

> **v6.24 追加（2026-06 / レイアウト修正・表紙刷新・グラデ活用）**
> - **考察頁の溢れ防止**（全文表示優先）／**チャート注釈の切れ解消**／**チャート＋注釈の左寄せ**。
> - **表紙=白基調**（白×赤斜線背景を転用・Century Gothic）。**発明スライドは普通の白背景**。
> - **グラデーション活用**（`grad_fill`/`_grad_text_fill`）: KPI強調・章扉番号/建築面・サブメッセージは廃止した左バーの代替。
> - **サブメッセージ**: 角丸・上下中央寄せ・1段拡大・先頭■記号と左赤バーを削除。
> - **コーナーマーク廃止**（`SHOW_CORNER_MARKS=False`）／**左端の章ガイドを復活**（`TOTAL_CHAPTERS` 既定修正）。
> - **2x2マトリクス等の枠線削除**／**ロードマップにカード間の右向き三角形＋文字拡大**。

> **v6.22 追加（2026-06 / デザイン微調整・クレーム品質）**
> - **注釈をグラフへさらに密着**: `add_chart_text_slide` の `gap` を 0.10→0.04。
> - **箇条書きの中点を黒に**: `_set_bullet` の既定色を C51212→111111（注釈の・は黒）。
> - **総括/締めの見出しラベルをパネルy中心へ**: `add_statement_slide` のラベルをパネル高にMIDDLEアンカーで縦中央配置。
> - **発明クレームの強化指針（重要）**: `add_invention_zone_slide` の `claimDraft` は『角度』で止めず、
>   **対象分野の劣化・破壊・反応の機構に紐づく測定可能な限定**で新規性・進歩性を確保する
>   （律速機構を特定 → それを断つ材料/構造を必須構成に → 機構指標で数値規定）。単なる配合・公知手段の組合せに留めない。
>   詳細手順は WORKFLOW §3.5 ステップ3、実例は同梱 example_content_co2.py を参照。

# PPTスライド仕様書 v6.21 — デザイン微調整＋deco廃止（自動コーナーマーク）

> **v6.21 追加（2026-06 / 実運用フィードバック反映・デザイン微調整）**
> - **deco 廃止（自動コーナーマーク）**: 明背景の各 `add_*_slide` が末尾で `add_corner_marks` を自動付与（冪等）。
>   content 側で `deco(...)` ラップは不要（後方互換のため no-op として残置可）。**`deco(...)` 二重括弧の閉じ忘れバグを根絶**。
> - **チャート横の注釈をネイティブ箇条書き化**: `add_annotation_block` は `■` マーカーを廃止し、
>   クリムゾンの buChar（ぶら下げインデント）で箇条書きに（`_set_bullet`）。
> - **PNG拡大＋注釈の寄せ**: `add_chart_text_slide` の `chart_ratio` を 0.68→0.715（画像 約+5%）、
>   チャートと注釈の `gap` を 0.25→0.10（注釈をグラフへ寄せる）。
> - **ネイティブ棒グラフのデータインク最大化（D2）**: `add_bar_chart_slide` に値ラベル直付け・主要目盛線除去・
>   冗長な数値軸の非表示を追加。
> - **意味マップ `anchor="auto"`（C）**: `add_semantic_map_slide` の callout は `anchor/box` を省略 or `"auto"` で
>   cid 重心から自動算出（scatter の正規化と一致＝ピルが正しく重心を指す）。
> - **総括/締めの半透明パネル**: `add_statement_slide` の白パネルを透明度20%（不透明度80%）へ、
>   見出しラベル（「何が分かったか」等）を 15.5→19.5pt に拡大。
...
> - 別セッションは「engine を exec → content.py を埋めて build_report.py 実行」で**同密度106枚を再生可能**（本リポで実測再現済み）。
> - CSVは content 冒頭で1回読み、各章レシピは WORKFLOW.md に明文化（章扉/ピラミッドはテンプレ関数を“使う”だけ）。
>
> **v6.17 追加（再現性の本丸・SKILL.md 受け入れ条件ゲートと連動）**
> - 別テーマでも同密度を再現するため、SKILL.md に **受け入れ条件ゲートG1〜G7**＋**標準デッキ・ブループリント（12章・各章最低枚数）** を新設。
> - 本格レポートは**概ね90枚以上（標準約100枚）**。50枚前後で止めるのは未完成。
> - 図解は分析（因果/比較軸/評価軸/戦略示唆）に対応するもののみ。**旧 `add_pyramid()`（台形積み重ね＝鏡持ち）を全面廃止**し、`add_pyramid_slide`（custGeom連続ピラミッド）に一本化。
> - 定量分析（**全系列一括ロジスティック回帰** `add_lifecycle_slide` ＋ **統計予測** `add_forecast_slide`）と、個別特許のミクロ精読（請求項・実施例・材料・製法・性能指標）を必須化。
>
> **v6.16 追加（目次の地力）**
...
> ②テーマフォント＝major: Century Gothic+Yu Mincho / minor: Century Gothic+Yu Gothic
> ③マスター署名＝天辺0.022inクリムゾン・ヘアライン（白頁全部に自動継承）
> ④暗背景スライドは `_hide_master(slide)`（showMasterSp="0"）でマスター図形を非表示（黒章扉・表紙・暗赤頁に透けない）


> **v6.13 追加**：発明アイディア(`add_invention_zone_slide`)のクレーム設計指針＝強いクレーム＝権利化(新規性/進歩性)×権利行使(検出可能/回避困難/輸入捕捉)の両立。**方法でなくモノ(装置/組成物/構造体)で構築し、製造方法は3案中1件まで**。差分軸に専門材料・数値パラメータを必須構成として入れる。詳細はSKILL.md。
...
> - **主要特許 深掘り** `add_patent_deepdive_slide`（書誌＋解決課題と、独立請求項の構成要件分解＋戦略的意義）。
> - **クロージング** `add_closing_slide` を v16 Template B に刷新（暗赤背景＋『分析による発見／仮説のアンサー／とるべき事業戦略』の3行）。
> - **結論ピラミッドを白背景化**（`add_pyramid_slide`）。クリムゾン縦グラデの連続ピラミッド＋左に淡色の論拠ボックス、見出し=墨。
> - 暗赤背景画像 `assets/dark_red_background.png` を同梱。`_cv_bg_image` / `_cv_panel` ヘルパー追加。色は赤/黒/白/グレーのみ。
...
## Section 0.5: マスター刷新 `tune_master`（最初に1回だけ実行）

> スライド生成前にテンプレート（apollo_template.pptx）へ1回適用する。4アーキタイプの設計思考：
> 表紙・章扉・結論＝フルブリード暗背景（マスター不可視＝スライド側 Crimson Vector が最適解）／
> コンテンツ頁（白・約8割）＝マスターが効く層。テーマとマスター署名で全頁の品質を底上げする。

```python
def tune_master(template_path):
    """テンプレートのテーマ色・テーマフォントを刷新し、マスターに署名ヘアラインを挿入する。"""
    import zipfile, re
    from pptx import Presentation
    from pptx.oxml.ns import qn
    from lxml import etree
    COLORS = {"dk1":"111111","lt1":"FFFFFF","dk2":"831010","lt2":"F6F7F8",
              "accent1":"C51212","accent2":"4D5055","accent3":"8F9499",
              "accent4":"C7CBD0","accent5":"831010","accent6":"E33333",
              "hlink":"C51212","folHlink":"831010"}
    with zipfile.ZipFile(template_path) as z:
        data = {n: z.read(n) for n in z.namelist()}
    theme = data["ppt/theme/theme1.xml"].decode("utf-8")
    for tag, hexv in COLORS.items():
        theme = re.sub(r'(<a:'+tag+r'>).*?(</a:'+tag+r'>)',
                       r'\1<a:srgbClr val="'+hexv+r'"/>\2', theme, flags=re.S)
    def _fonts(block, latin, ea):
        block = re.sub(r'(<a:latin typeface=")[^"]*(")', r'\g<1>'+latin+r'\2', block, count=1)
        return re.sub(r'(<a:ea typeface=")[^"]*(")', r'\g<1>'+ea+r'\2', block, count=1)
    mj = re.search(r'<a:majorFont>.*?</a:majorFont>', theme, re.S).group(0)
    mn = re.search(r'<a:minorFont>.*?</a:minorFont>', theme, re.S).group(0)
    theme = theme.replace(mj, _fonts(mj, "Century Gothic", "Yu Mincho"))
    theme = theme.replace(mn, _fonts(mn, "Century Gothic", "Yu Gothic"))
    data["ppt/theme/theme1.xml"] = theme.encode("utf-8")
    with zipfile.ZipFile(template_path, "w", zipfile.ZIP_DEFLATED) as z:
        for n, b in data.items():
            z.writestr(n, b)
    prs = Presentation(template_path)
    spTree = prs.slide_masters[0].shapes._spTree
    if not any("BrandHairline" in etree.tostring(sp, encoding=str)[:300]
               for sp in spTree.findall(qn('p:sp'))):
        spTree.append(etree.fromstring(
          '<p:sp xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main" '
          'xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">'
          '<p:nvSpPr><p:cNvPr id="900" name="BrandHairline"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>'
          '<p:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="12192000" cy="20320"/></a:xfrm>'
          '<a:prstGeom prst="rect"><a:avLst/></a:prstGeom>'
          '<a:solidFill><a:srgbClr val="C51212"/></a:solidFill>'
          '<a:ln><a:noFill/></a:ln><a:effectLst/></p:spPr>'
          '<p:txBody><a:bodyPr/><a:lstStyle/><a:p/></p:txBody></p:sp>'))
    prs.save(template_path)
```

> **必須対**：暗背景を描く全スライド型（表紙・章扉・3点ステートメント・暗背景発明等）は
> `_hide_master(slide)` を呼びマスター図形を非表示にする（実装済み）。
