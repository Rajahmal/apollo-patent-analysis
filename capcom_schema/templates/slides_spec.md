...
- [ ] **数値の出所統一**: 全数値はセッションの分析データ（JSON）・`patents.csv` を唯一の正準とする。
  転記元レポート等の再計算値とJSONが食い違う場合は**JSONを採用**し、シェアの母数（全体母集団など）も
  全スライドで統一する。代表特許の番号・名称はCSVの実値（公開・公表番号）を用い、推定で書かない。
